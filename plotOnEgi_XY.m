...
set(plotH,'facevertexCdata',data,'linewidth',0.5,'markersize',10,'marker','.');
...
if sensorROI{1} ~= 0 
    for t = 1:length(sensorROI)
        ...
        if doText
            ...
            text(roiX,roiY,roiZ,num2str(sensorROI{t}),'fontsize',14,'fontname','Arial','horizontalAlignment','center')
%             arrayfun(@(x) ...
%                 text(roiX(x),roiY(x),roiZ(x), num2str(x),'fontsize',16,'fontname','Arial','horizontalAlignment','center')...
