%% CARRIERS: OVERLAY ALL 5 GROUPS IN ONE PLOT 
clear;clc;close all;
...
figure('position',[100 100 550 400],'color','w')
% subplot_tight(1,3,1,[0.2 0.06])
hold on;
for run = [1:4,1]
    ...
    if run == 3
        plot(meand,'color',[mycolor(run,:) 0.8],'linewidth',1.8,'LineStyle','-');
    elseif run == 4
        plot(meand,'color',[mycolor(run,:) 0.6],'linewidth',1.8,'LineStyle','-');
    else
...
mylegend = legend;
legend({'3-4 months','4-6 months','6-8 months','12-15 months'},'fontsize',14)
legend boxoff
...
figpath = bbpath;
print(gcf,[figpath,'Timedomain-',roiname,'-overlay-4Groups-Carrier-pc-new.tiff'],'-dtiff','-r300')


%% CARRIERS: OVERLAY ALL 5 GROUPS IN ONE PLOT  - 5 folds
% clear;clc;close all;
% figpath = '/oak/stanford/groups/kalanit/biac2/kgs/projects/babybrains/eeg/data/prelimresults/2021 dataset bb/arranged_dataset/';
...
figure('position',[100 100 200 400],'color','w')
% subplot_tight(1,3,2,[0.2 0.06])
hold on;
for run = [1:4,1]
%     load('New3Groups_arrange_no_concat_carrier.mat');
    if run == 1
    ...
    if run == 3
        plot(xxtt,meand,'color',[mycolor(run,:) 0.8],'linewidth',1.8,'LineStyle','-');
    elseif run == 4
        plot(xxtt,meand,'color',[mycolor(run,:) 0.6],'linewidth',1.8,'LineStyle','-');
    else
        plot(xxtt,meand,'color',mycolor(run,:),'linewidth',1.8,'LineStyle','-');
    end
    ...
    b1 = scatter(xxtt(find(corrH)),-thresh+run*0.3*ones(length(find(corrH)),1),2,mycolor(run,:),'*');
    b1.Annotation.LegendInformation.IconDisplayStyle = 'off';
% 
% %     %find peak in each waveform
%     if roi == 2
%         for time2 = 60   
%             tp2 = round(time2*420/1000,0);
%             peakdata = meand(1:tp2);
%             [findmin_value,findmin_ix] = min(peakdata);
%             pmin = plot([xxtt(findmin_ix) xxtt(findmin_ix)],[-thresh thresh],'color',mycolor(run,:),'linestyle','--','linewidth',1.2);
%             pmin.Annotation.LegendInformation.IconDisplayStyle = 'off';
%         end
%         for time2 = 100
%             tp2 = round(time2*420/1000,0);
%             peakdata = meand(1:tp2);
...
%             peakdata = meand(1:tp2);
%             [findmax_value,findmax_ix] = max(peakdata);
%             pmax = plot([xxtt(findmax_ix) xxtt(findmax_ix)],[-thresh thresh],'color',mycolor(run,:),'linestyle','--','linewidth',1.2);
%             pmax.Annotation.LegendInformation.IconDisplayStyle = 'off';
%         end
%     end
end
% hold on;
% p = plot([60 60],[-thresh thresh],'linewidth',1.5,'color',[0.5 0.5 0.5],'linestyle','--')
%     p.Annotation.LegendInformation.IconDisplayStyle = 'off';
% p = plot([90 90],[-thresh thresh],'linewidth',1.5,'color',[0.5 0.5 0.5],'linestyle','--')
%     p.Annotation.LegendInformation.IconDisplayStyle = 'off';
% p = plot([160 160],[-thresh thresh],'linewidth',1.5,'color',[0.5 0.5 0.5],'linestyle','--')
%     p.Annotation.LegendInformation.IconDisplayStyle = 'off';

% addtp = 1;
% if addtp == 1
%     hold on;
%     p = plot([0 233],[-thresh+0.5 -thresh+0.5],'linewidth',1.5,'color','k')
%     p.Annotation.LegendInformation.IconDisplayStyle = 'off';
% end
% legend({'3-4 months','4-6 months','6-8 months','12-15 months','adults'},'fontsize',12)
% legend({'3-4 months','4-6 months','6-8 months','12-15 months'},'fontsize',12,'location','northwest')
...
xlabel('Time (ms)')
ylabel('Amplitude (µV)')
h = plot([0,233],[0,0],'color',[0.5 0.5 0.5],'linewidth',1,'LineStyle','--');
h.Annotation.LegendInformation.IconDisplayStyle = 'off';
set(gca,'fontsize',16,'linewidth',1.3)
%     title(lists{i},'fontweight','normal')
%     title(sprintf('%d-month',months),'fontweight','normal','fontsize',18)
% sgtitle(roiname,'fontsize',18)
figpath = bbpath;
print(gcf,[figpath,'Timedomain-',roiname,'-overlay-4Groups-Carrier-5folds-pc-new.tiff'],'-dtiff','-r300')
% figname = sprintf('Timedomain-%s-overlay-4Groups-Carrier-5folds-timeWin-stats-withEdgechan.png',roiname)
% saveas(gcf,fullfile(figpath,figname))

%% CARRIERS: 5 GROUPS IN SUBPLOTS, GROUP LEVEL
clear;clc;close all;
figpath = '/oak/stanford/groups/kalanit/biac2/kgs/projects/babybrains/eeg/data/prelimresults/2021 dataset bb/arranged_dataset/';
cd(figpath)
LeftOT_bb = [50 57 58 59 64 65]; %removed chan68,73
...
hold on;
for run = 1:4
    load('New3Groups_arrange_clean_concat_carrier.mat');
    if run == 1
        newData = groupData1;
        subjList_short = idx1;
        groupname = 'Group1-3-4';
    ...
    subplot_tight(1,4,run)

    for roi = 1:2
        hold on;
        if roi == 1
            rois = OTROI; roiname = 'OTROI'
            mycolor = repmat([0.5 0.5 0.5],5,1);
        elseif roi == 2
            rois = Posterior_bb; roiname = 'OCCROI'
            mycolor = [161,218,180;...
                65,182,196;...
    ...
    legend({'Occipito-temporal','Posterior medial'},'fontsize',12)
    legend boxoff
    addtp = 1; %stimulus onset duration
    if addtp == 1
        time1 = 0;
        time2 = 233;
        tp1 = 420*time1/1000;
    ...
    h.Annotation.LegendInformation.IconDisplayStyle = 'off';
    set(gca,'fontsize',16,'linewidth',1.3)
    title(grouplists{run},'fontweight','normal','fontsize',18)
%     title(sprintf('%d-month',months),'fontweight','normal','fontsize',18)
end
print(gcf,[figpath,'Timedomain-',roiname,'-subplots-4Groups-Carrier.tiff'],'-dtiff','-r300')
...
% figure('position',[100 100 1600 400],'color','w')
for run = 1:4
    if run == 5
        figure('position',[100 100 1600 400],'color','w')
    end
...
%     sgtitle(roiname,'fontsize',18)
    figpath = 'D:\2023NSFC\';
    print(gcf,fullfile(figpath,sprintf('Timedomain-oddball-%s-30Hz-group%d-%sFDR.tiff',roiname,run,lists{i})),'-dtiff','-r300')
    close
end
...
for run = 1:4
    if run == 1
        ...
        subjList_short = idx1;
        groupname = 'Group1-3-4';
        figure('position',[100 100 1800 300*5],'color','w')
    elseif run == 2
        newData = groupData2;
        subjList_short = idx2;
        groupname = 'Group2-4-6';
        figure('position',[100 100 1800 300*3],'color','w')
    elseif run == 3
        newData = groupData3;
        subjList_short = idx3;
        groupname = 'Group3-6-8';
    ...
    for i = 1:size(mVdataIn,1)
        ...
        % win 1
        if run < 4
            [mval,mlatency] = timeDomain_peak_metrics(mtemp,65,100,'neg');
        else
            [mval,mlatency] = timeDomain_peak_metrics(mtemp,65,90,'neg');
        end
        ...
        for n = 1:nsubj
            ...
            if i == 1
                if run == 1
                    time1 = 400
                    time2 = 800
                    [mval,mlatency] = timeDomain_peak_metrics(mtemp,400,800,'pos');
                else
                    time1 = 300
                    time2 = 700
                    [mval,mlatency] = timeDomain_peak_metrics(mtemp,300,700,'pos');
                end
            elseif i >= 3
                time1 = 400
                time2 = 800
...
%                 time1 = 300
%                 time2 = 700
%                 [mval,mlatency] = timeDomain_peak_metrics(mtemp,300,700,'neg');
            end
