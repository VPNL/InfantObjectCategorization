...
%     subplot_tight(1,2,roi,[0.15,0.06])
...
%% breakdown interaction of hemi x age
% look at rightHemi first
con = 4;
roiname = 'RightOTROI';
% roiname = 'LeftOTROI'
load(sprintf('TimeDomain_individual_oddball_%s_win_stats_%s_allgroups.mat',conlists{con},roiname));

winlists = {'300-700 ms'};
figure('position',[300 300 400 300],'color','w');
ii = 0;
for metric = {'amp'} %{'amp' 'latency'}  %amp or latency
    ...
    elseif strmatch(metric,'latency')
        data = allsubj_info_latency(:,con,:);
    end
    hold on;
    scatter(group1',data(1,1:length(group1)),40,mycolor(1,:),'filled');
    scatter(group2',data(2,1:length(group2)),40,mycolor(2,:),'^','filled');
    scatter(group3',data(3,1:length(group3)),45,mycolor(3,:),'*');
    scatter(group4',data(4,1:length(group4)),40,mycolor(4,:),'d','filled');
    xlim([60 500])
    xticks([60:30:500])
    xtickangle(90)
    ...
    tbl = table(com_Age,com_Amp',neworder','VariableNames',{'Age','PeakResponse','Baby'});
    lme1 = fitlme(tbl,'PeakResponse~ Age +(1|Baby)')
    model(con).lme1 = lme1;
    x = 60:500; y = lme1.Coefficients.Estimate(1) + (lme1.Coefficients.Estimate(2))*x;

    %predix model
    newtbl= table(x', y', x' ,'VariableNames',{'Age','PLatency','Baby'});
    [Ypred,YCI] = predict(lme1,newtbl);
    [val order] = sort(x);
    plot(val,y(order), 'color', 'r','linewidth',1.5);
    patch([val, fliplr(val)],[YCI(order,1)', fliplr(YCI(order,2)')],[1 1 1],'edgecolor', [1 1 1], 'facecolor', 'r', 'FaceAlpha', 0.2)
    ...
    %save
    filename =  sprintf('TimeDomain_oddb_%s_%s_AcrossAges_lmm_stats_%s.mat',metric{:},roiname,conlists{con});
    save(fullfile(bbpath,filename),'lme1');
    figname = sprintf('TimeDomain_oddb_%s_%s_AcrossAges_withlmm_%s.png',metric{:},roiname,conlists{con});
    saveas(gcf,fullfile(bbpath,figname));
end
...
%% left OT： interaction of category x age
...
%% right OT: interaction of category x age
...
%face test
tbl1 = tbl(1:116,:);
lme_test1 = fitlme(tbl1,'PeakResp~ Age*Hemi +(1|Baby)')
%car test
tbl2 = tbl(117:116*2,:);
lme_test2 = fitlme(tbl2,'PeakResp~ Age*Hemi +(1|Baby)')
%corri test
tbl3 = tbl(116*2+1:116*3,:);
lme_test3 = fitlme(tbl3,'PeakResp~ Age*Hemi +(1|Baby)')
%limb test
tbl4 = tbl(116*3+1:116*4,:);
lme_test4 = fitlme(tbl4,'PeakResp~ Age*Hemi +(1|Baby)')
